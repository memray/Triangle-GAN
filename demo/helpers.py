...
def plot_connect(orig, z_batch, a, b, c, d, dir=None, filename="z"):
    ...
    for n in range(z_batch.shape[0]):
        ...
        result = pylab.scatter(orig[n, 0], 1, c=colors[np.floor_divide(n, 1 + (np.floor_divide(z_batch.shape[0], 10)))],
                               s=40, marker="o", edgecolors='none')
        result = pylab.scatter(z_batch[n, 0], z_batch[n, 1],
                               c=colors[np.floor_divide(n, 1 + (np.floor_divide(z_batch.shape[0], 10)))], s=40,
...
def plot_labeled_z(z_batch, label_batch, dir=None, filename="labeled_z"):
    ...
    colors = ["#2103c8", "#0e960e", "#e40402", "#05aaa8", "#ac02ab", "#aba808", "#151515", "#94a169", "#bec9cd",
              "#6a6551"]
    for n in range(z_batch.shape[0]):
        result = pylab.scatter(z_batch[n, 0], z_batch[n, 1], c=colors[label_batch[n]], s=40, marker="o",
                               edgecolors='none')
