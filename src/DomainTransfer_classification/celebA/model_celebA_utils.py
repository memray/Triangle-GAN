...
def encoder1(tensor):
    ...
    conv2 = layers.conv2d(conv1, 64, 4, stride=2, activation_fn=None, normalizer_fn=layers.batch_norm,
    ...
    fc2 = layers.fully_connected(inputs=fc1, num_outputs=40, activation_fn=tf.nn.sigmoid,
                                 weights_initializer=initializer)
...
def encoder2(y, z):
    ...
    h = layers.conv2d_transpose(h, 64 * 4, 5, stride=2, padding='SAME', activation_fn=None,
                                weights_initializer=initializer)
    h = layers.batch_norm(h, activation_fn=lrelu)
    # h = conv_cond_concat(h, yb)

    h = layers.conv2d_transpose(h, 64 * 2, 5, stride=2, padding='SAME', activation_fn=None,
                                weights_initializer=initializer)
    h = layers.batch_norm(h, activation_fn=lrelu)
    # h = conv_cond_concat(h, yb)

    h = layers.conv2d_transpose(h, 64 * 1, 5, stride=2, padding='SAME', activation_fn=None,
                                weights_initializer=initializer)
    h = layers.batch_norm(h, activation_fn=lrelu)

    # h = conv_cond_concat(h, yb)
    h = layers.conv2d_transpose(h, 3, 5, stride=2, padding='SAME', activation_fn=tf.nn.tanh,
                                weights_initializer=initializer)
    return h
