...
def discriminator(x, y):
    ...
    h = tf.contrib.layers.conv2d(h, 64, 5, stride=2, padding='SAME', activation_fn=None,
                                 weights_initializer=initializer)
    h = tf.contrib.layers.batch_norm(h, activation_fn=tf.nn.relu)

    # h = conv_cond_concat(h, yb)
    h = tf.contrib.layers.conv2d(h, 64 * 2, 5, stride=2, padding='SAME', activation_fn=None,
                                 weights_initializer=initializer)
    h = tf.contrib.layers.batch_norm(h, activation_fn=tf.nn.relu)

    # h = conv_cond_concat(h, yb)
    h = tf.contrib.layers.conv2d(h, 64 * 4, 5, stride=2, padding='SAME', activation_fn=None,
                                 weights_initializer=initializer)
    h = tf.contrib.layers.batch_norm(h, activation_fn=tf.nn.relu)

    h = conv_cond_concat(h, yb)
    h = tf.contrib.layers.conv2d(h, 64 * 8, 5, stride=2, padding='SAME', activation_fn=tf.nn.relu,
                                 weights_initializer=initializer)
    h = tf.contrib.layers.batch_norm(h, activation_fn=tf.nn.relu)
