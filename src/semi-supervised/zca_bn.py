...
class MeanOnlyBNLayer(lasagne.layers.Layer):
        ...
        if modify_incoming:
            ...
            if g is not None:
                incoming.W = incoming.W_param * (self.g / T.sqrt(
                    T.sum(T.square(incoming.W_param), axis=W_axes_to_sum, dtype=th.config.floatX,
                          acc_dtype=th.config.floatX))).dimshuffle(*W_dimshuffle_args)
            else:
                incoming.W = incoming.W_param / T.sqrt(
                    T.sum(T.square(incoming.W_param), axis=W_axes_to_sum, keepdims=True, dtype=th.config.floatX,
    ...
    def get_output_for(self, input, deterministic=False, init=False, **kwargs):
        if deterministic:
            ...
            if init:
                stdv = T.sqrt(T.mean(T.square(activation), axis=self.axes_to_sum, dtype=th.config.floatX,
                                     acc_dtype=th.config.floatX))
