...
from lasagne.layers import InputLayer, ReshapeLayer, FlattenLayer, Upscale2DLayer, MaxPool2DLayer, DropoutLayer, \
    ConcatLayer, DenseLayer, NINLayer
from lasagne.layers import GaussianNoiseLayer, Conv2DLayer, Pool2DLayer, GlobalPoolLayer, DimshuffleLayer, ElemwiseSumLayer
from lasagne.utils import floatX
from zca_bn import ZCA
from zca_bn import mean_only_bn as WN

import os, cPickle, time, math, argparse, shutil, sys
...
from components.objectives import maximum_mean_discripancy, \
    categorical_crossentropy, feature_matching
...
if True:
    print
    'Size of training data', x_labelled.shape[0], x_unlabelled.shape[0]
    y_order = np.argsort(y_labelled)
    _x_mean = x_labelled[y_order]
    image = paramgraphics.mat_to_img(_x_mean.T, dim_input, tile_shape=(num_classes, num_labelled / num_classes),
                                     colorImg=colorImg, scale=generation_scale,
                                     save_path=os.path.join(outfolder, 'x_l_' + str(ssl_data_seed) + '_triple-gan.png'))
...
gen_layers.append(nn.batch_norm(
    ll.DenseLayer(gen_layers[-1], num_units=4 * 4 * 512, W=Normal(0.05), nonlinearity=nn.relu, name='gen-01'), g=None,
    name='gen-02'))
...
gen_layers.append(nn.batch_norm(
    nn.Deconv2DLayer(gen_layers[-1], (None, 256, 8, 8), (5, 5), W=Normal(0.05), nonlinearity=nn.relu, name='gen-11'),
    g=None, name='gen-12'))  # 4 -> 8
gen_layers.append(ConvConcatLayer([gen_layers[-1], gen_in_y], num_classes, name='gen-20'))
gen_layers.append(nn.batch_norm(
    nn.Deconv2DLayer(gen_layers[-1], (None, 128, 16, 16), (5, 5), W=Normal(0.05), nonlinearity=nn.relu, name='gen-21'),
    g=None, name='gen-22'))  # 8 -> 16
gen_layers.append(ConvConcatLayer([gen_layers[-1], gen_in_y], num_classes, name='gen-30'))
gen_layers.append(nn.weight_norm(
    nn.Deconv2DLayer(gen_layers[-1], (None, 3, 32, 32), (5, 5), W=Normal(0.05), nonlinearity=gen_final_non,
                     name='gen-31'), train_g=True, init_stdv=0.1, name='gen-32'))  # 16 -> 32
...
dis_layers.append(nn.weight_norm(
    dnn.Conv2DDNNLayer(dis_layers[-1], 32, (3, 3), pad=1, W=Normal(0.05), nonlinearity=nn.lrelu, name='dis-02'),
    name='dis-03'))
dis_layers.append(ConvConcatLayer([dis_layers[-1], dis_in_y], num_classes, name='dis-20'))
dis_layers.append(nn.weight_norm(
    dnn.Conv2DDNNLayer(dis_layers[-1], 32, (3, 3), pad=1, stride=2, W=Normal(0.05), nonlinearity=nn.lrelu,
                       name='dis-21'), name='dis-22'))
...
dis_layers.append(nn.weight_norm(
    dnn.Conv2DDNNLayer(dis_layers[-1], 64, (3, 3), pad=1, W=Normal(0.05), nonlinearity=nn.lrelu, name='dis-31'),
    name='dis-32'))
dis_layers.append(ConvConcatLayer([dis_layers[-1], dis_in_y], num_classes, name='dis-40'))
dis_layers.append(nn.weight_norm(
    dnn.Conv2DDNNLayer(dis_layers[-1], 64, (3, 3), pad=1, stride=2, W=Normal(0.05), nonlinearity=nn.lrelu,
                       name='dis-41'), name='dis-42'))
...
dis_layers.append(nn.weight_norm(
    dnn.Conv2DDNNLayer(dis_layers[-1], 128, (3, 3), pad=0, W=Normal(0.05), nonlinearity=nn.lrelu, name='dis-51'),
    name='dis-52'))
dis_layers.append(ConvConcatLayer([dis_layers[-1], dis_in_y], num_classes, name='dis-60'))
dis_layers.append(nn.weight_norm(
    dnn.Conv2DDNNLayer(dis_layers[-1], 128, (3, 3), pad=0, W=Normal(0.05), nonlinearity=nn.lrelu, name='dis-61'),
    name='dis-62'))
...
dis_layers.append(
    nn.weight_norm(ll.DenseLayer(dis_layers[-1], num_units=1, W=Normal(0.05), nonlinearity=ln.sigmoid, name='dis-71'),
...
dis2_layers.append(nn.weight_norm(
    dnn.Conv2DDNNLayer(dis2_layers[-1], 32, (3, 3), pad=1, W=Normal(0.05), nonlinearity=nn.lrelu, name='dis2-02'),
    name='dis2-03'))
dis2_layers.append(ConvConcatLayer([dis2_layers[-1], dis2_in_y], num_classes, name='dis2-20'))
dis2_layers.append(nn.weight_norm(
    dnn.Conv2DDNNLayer(dis2_layers[-1], 32, (3, 3), pad=1, stride=2, W=Normal(0.05), nonlinearity=nn.lrelu,
                       name='dis2-21'), name='dis2-22'))
...
dis2_layers.append(nn.weight_norm(
    dnn.Conv2DDNNLayer(dis2_layers[-1], 64, (3, 3), pad=1, W=Normal(0.05), nonlinearity=nn.lrelu, name='dis2-31'),
    name='dis2-32'))
dis2_layers.append(ConvConcatLayer([dis2_layers[-1], dis2_in_y], num_classes, name='dis2-40'))
dis2_layers.append(nn.weight_norm(
    dnn.Conv2DDNNLayer(dis2_layers[-1], 64, (3, 3), pad=1, stride=2, W=Normal(0.05), nonlinearity=nn.lrelu,
                       name='dis2-41'), name='dis2-42'))
...
dis2_layers.append(nn.weight_norm(
    dnn.Conv2DDNNLayer(dis2_layers[-1], 128, (3, 3), pad=0, W=Normal(0.05), nonlinearity=nn.lrelu, name='dis2-51'),
    name='dis2-52'))
dis2_layers.append(ConvConcatLayer([dis2_layers[-1], dis2_in_y], num_classes, name='dis2-60'))
dis2_layers.append(nn.weight_norm(
    dnn.Conv2DDNNLayer(dis2_layers[-1], 128, (3, 3), pad=0, W=Normal(0.05), nonlinearity=nn.lrelu, name='dis2-61'),
    name='dis2-62'))
...
dis2_layers.append(
    nn.weight_norm(ll.DenseLayer(dis2_layers[-1], num_units=1, W=Normal(0.05), nonlinearity=ln.sigmoid, name='dis2-71'),
                   train_g=True, init_stdv=0.1, name='dis2-72'))
...
dis_out_p = ll.get_output(dis_layers[-1], {dis_in_x: T.concatenate([sym_x_l, sym_x_u_d], axis=0),
...
cla_cost = alpha_cla_adv * (cla_cost_p_c + cla2_cost_p_c) + alpha_cla * (
cla_cost_l + cla_cost_u) + sym_alpha_cla_g * cla_cost_cla_g
# fast
...
cla_cost_list = [cla_cost, alpha_cla_adv * (cla_cost_p_c + cla2_cost_p_c), alpha_cla * cla_cost_l,
                 alpha_cla * cla_cost_u, sym_alpha_cla_g * cla_cost_cla_g]
# fast
cla_cost_list_fast = [cla_cost_fast, alpha_cla_adv * (cla_cost_p_c + cla2_cost_p_c), alpha_cla * cla_cost_l,
                      alpha_cla * cla_cost_u, ]
...
# fast updates of C
cla_params = ll.get_all_params(classifier, trainable=True)
cla_updates_fast_ = robust_adam(cla_cost_fast, cla_params, learning_rate=sym_lr_cla, beta1=sym_b_c, beta2=.999,
                                epsilon=1e-8)
...
train_batch_cla = theano.function(
    inputs=[sym_x_l, sym_y, sym_y_g, slice_x_u_c, sym_alpha_cla_g, sym_lr_cla, sym_b_c, sym_unsup_weight],
...
for epoch in range(1, 1 + num_epochs):
    ...
    for i in range(n_batches_train_u_c):
        ...
        tmp = time.time()
        dl_b = train_batch_dis(x_labelled[from_l_d:to_l_d], y_labelled[from_l_d:to_l_d], sample_y, p_u[from_u_c:to_u_c],
                               p_u_d[from_u_d:to_u_d], lr)
        ...
        if epoch < epoch_cla_g:
            cl_b = train_batch_cla_fast(x_labelled[from_l_c:to_l_c], y_labelled[from_l_c:to_l_c], p_u[from_u_c:to_u_c],
                                        lr_c, b1_c, unsup_weight)
            cl_b += [0, ]
        else:
            cl_b = train_batch_cla(x_labelled[from_l_c:to_l_c], y_labelled[from_l_c:to_l_c], sample_y,
    ...
    print
    'dis:', dis_t, 'dis2:', dis2_t, 'gen:', gen_t, 'cla:', cla_t, 'total', dis_t + dis2_t + gen_t + cla_t
    ...
    line = "*Epoch=%d Time=%.2f LR=%.5f\n" % (epoch, t, lr) + "DisLosses: " + str(dl) + "DisLosses2: " + str(
        dl2) + "\nGenLosses: " + str(gl) + "\nClaLosses: " + str(cl)
